...
**「TradingViewとEA Studioを融合させ、それぞれの長所を取り入れながら、それらを超える自由度・拡張性・分析能力を持つストラテジー研究プラットフォーム」**

要点:

- **特定の手法専用ソフトではない。** あらゆるストラテジーを組める汎用プラットフォームが完成形。ショート専用/USDJPY専用/EMA専用/固定パラメータのような設計は禁止。
- **ロング/ショート/両方を自由に切り替え可能に。** ショート専用設計は明示的に禁止。
- **条件はAND/OR/NOTで自由に組み合わせ、数の制限を作らない。** 将来数百種類の条件を想定し、追加しやすい設計にする。
- **パラメータは全て最適化可能な範囲として設計。** 固定値での実装は禁止。
- **設計の優先順位: 拡張性 > 保守性 > 速度 > 可読性。** 多少コード量が増えても後から機能追加しやすい設計を優先。
- **ロジックとUIは分離。**
- 通貨/銘柄・時間足・資産クラス（株/指数/ゴールド/仮想通貨）・AIによる自動探索・エントリー方式/決済方式/リスク管理の多様化は「将来的に」の対応でよいが、今の設計がそれを妨げないようにする。

**2026-07-05に着手した対応(条件エンジン化):** 上記を受けて`engine/conditions.py`を新設。`Condition`/`ConditionGroup`によるAND/OR/NOT自由結合の条件ツリーエンジンで、指標名は`INDICATOR_REGISTRY`経由(現状close/open/high/low/candle_body/ema/rsi/highest_high/lowest_low/donchian_mid/bollinger系/macd系/stochastic系/hour/weekday)。方向(direction: short/long)は条件の内容から独立した機械的なプロパティとして`engine/backtest_engine.py::run_backtest()`に追加(エントリー確認・SL/TP・利益計算の4箇所のみ分岐)。**既存の`engine/triggers.py`/`engine/filters.py`/`build_candidate_signal`は一切変更していない**(`params`に`condition_tree`キーがある場合のみ新エンジンを使う追加的な実装で、既存の回帰テストは無修正のまま全てPASS)。GUIに「条件ビルダー」タブを新設し、指標・演算子・しきい値をチェックボックスで組み立ててLong/Short方向を選び実行できる(`strategy_configs/*.json`経由で既存の`--strategy-config`の仕組みをそのまま再利用)。

現時点で未着手(完成形の「将来的に」項目、設計上ブロックはしていない): 既存16トリガー/22フィルターの新エンジンへの移行、SMC Tier3のLong側ミラー、direction="both"、GUIでのOR/NOTネスト、エントリー方式/決済方式/リスク管理のプラガブル化、マルチタイムフレーム分析、FX以外の資産クラス、AIによる自動探索。
...
**Version 2.0 / 3.0 は完了。Version 4.0 も主要機能は完了（v4.0.0タグ済み、2026-07-04）。ライセンス/課金とExcel出力のみ未着手。**
...
v1.0.0, v3.0.0, v4.0.0（2026-07-04付、GUI/PDF/パッケージ化/複数通貨対応/データパイプライン刷新まで含む。現時点の最新タグ）
