...
def reconstruct_params_from_row(row: dict[str, Any]) -> dict:
    return {
        ...
        "use_atr_trailing_stop": bool(row.get("use_atr_trailing_stop", False)),
        "atr_trailing_length": int(row.get("atr_trailing_length", 14)),
        "atr_trailing_multiplier": float(row.get("atr_trailing_multiplier", 2.0)),
        "use_max_dd_stop": bool(row.get("use_max_dd_stop", False)),
        "max_dd_stop_pips": float(row.get("max_dd_stop_pips", 100.0)),
        ...
        "use_position_sizing": bool(row.get("use_position_sizing", False)),
        "position_sizing_method": str(row.get("position_sizing_method", "risk_percent")),
        "initial_capital": float(row.get("initial_capital", 1_000_000.0)),
        "account_currency": str(row.get("account_currency", "JPY")),
        "risk_percent": float(row.get("risk_percent", 1.0)),
        "fixed_lot_size": float(row.get("fixed_lot_size", 0.1)),
        "contract_size": float(row.get("contract_size", 100_000.0)),
        "conversion_rate": float(row.get("conversion_rate", 150.0)),
        "use_breakeven_stop": bool(row.get("use_breakeven_stop", False)),
        "breakeven_trigger_rr": float(row.get("breakeven_trigger_rr", 0.5)),
        ...
        "sl_basis": str(row.get("sl_basis", "signal_candle")),
        "sl_atr_length": int(row.get("sl_atr_length", 14)),
        "sl_atr_multiplier": float(row.get("sl_atr_multiplier", 2.0)),
        "sl_fixed_pips": float(row.get("sl_fixed_pips", 20.0)),
        "tp_basis": str(row.get("tp_basis", "rr")),
        "tp_fixed_pips": float(row.get("tp_fixed_pips", 20.0)),
        "exit_condition_tree": _reconstruct_tree(row, "exit_condition_tree"),
