...
All five are implemented as BEARISH-only variants, matching the existing
strategy's short-only direction (every trigger/filter in this engine marks
candidate SHORT setups) - see engine/backtest_engine.py's profit
calculation (entry_price - exit_price), which is short-only.
